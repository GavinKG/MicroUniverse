...
![image-20200724174038949](README.assets/image-20200724174038949.png)
...
* 对于每个变换后的内部区域，从交汇点开始（可能有多个交汇点）用 WFC 算出路网/中心喷泉（十字路口会出现）/能量柱布局。详细布局信息参考单元测试；
* 路网周围随机生成房屋，一个格一个房屋。
* （可选）连在一起的两个/三个房屋合并为一个长条形房屋。
* 将建筑顶点变换回笛卡尔坐标系（可以选择变换哪些），喷泉/能量柱直接变换中心点即可。靠城市墙壁和及建筑的碰撞形成可行走的道路；
...
- [x] WFC
  ...
  >   * Generating Adjacency rules based on **3x3 Overlapping Model** (for better accuracy)
