...
namespace AmplifyShaderEditor
{
	...
	public class TemplateDataCollector
	{
		...
		public string GetTextureCoord( int uvChannel, string propertyName, int uniqueId, PrecisionType precisionType )
		{
			...
			string result = string.Empty;
			if( GetCustomInterpolatedData( TemplateHelperFunctions.IntToUVChannelInfo[ uvChannel ], WirePortDataType.FLOAT4, PrecisionType.Float, ref result, false, m_currentDataCollector.PortCategory ) )
			{
				result += UIUtils.GetAutoSwizzle( WirePortDataType.FLOAT2 );
				uvName = result;
			}
			else
		...
		public string GenerateAutoUVs( int uvChannel, WirePortDataType size = WirePortDataType.FLOAT2 )
		{
			...
			if( GetCustomInterpolatedData( TemplateHelperFunctions.IntToUVChannelInfo[ uvChannel ], WirePortDataType.FLOAT4, PrecisionType.Float, ref result, false, m_currentDataCollector.PortCategory ) )
			{
				if( size != WirePortDataType.FLOAT4 )
					result += UIUtils.GetAutoSwizzle( size );
				uvName = result;
			}
			else
			if( HasUV( uvChannel ) )
